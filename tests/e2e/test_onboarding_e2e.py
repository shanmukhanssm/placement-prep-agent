...
ONBOARDING_TURNS: list[tuple[str, dict[str, object]]] = [
    ...
    (
        "SDE, data analyst",
        {"message": "Weak areas?", "extracted": {"target_roles": "SDE, data analyst"}},
    ),
    (
        "arrays, dynamic programming",
        {
            "message": "Core subject: aiml or cyber?",
    ...
    (
        "aiml",
        {
            "message": "You're all set, Arjun — welcome aboard!",
            "extracted": {"core_subject": "aiml"},
        },
    ),
]
...
def test_returning_user_thread_skips_onboarding(llm_queues, tmp_path):
    ...
    app = build_graph(make_sqlite_checkpointer(str(tmp_path / "cp.sqlite")))
    _invoke(
        app, {"configurable": {"thread_id": "t1"}, "recursion_limit": RECURSION_LIMIT}, "hi there!"
    )
    for message, _canned in ONBOARDING_TURNS[1:]:
        _invoke(
            app, {"configurable": {"thread_id": "t1"}, "recursion_limit": RECURSION_LIMIT}, message
        )
    ...
    result = _invoke(
        app,
        {"configurable": {"thread_id": "t2"}, "recursion_limit": RECURSION_LIMIT},
        "hello again",
    )
...
def test_onboarding_tool_failure_keeps_state(llm_queues, tmp_path, monkeypatch):
    ...
    app = build_graph(make_sqlite_checkpointer(str(tmp_path / "cp.sqlite")))
    config = {
        "configurable": {"thread_id": "test:onboarding-fail"},
        "recursion_limit": RECURSION_LIMIT,
    }
    result = None
...
def test_onboarding_reasks_invalid_grad_year(llm_queues, tmp_path):
    ...
    turns[3] = {
        "message": "A number please — e.g. 2027",
        "extracted": {"grad_year": "in two years"},
    }
    turns.insert(4, {"message": "2027, got it!", "extracted": {"grad_year": "2027"}})
    llm_queues["onboarding_collector"] = turns
    app = build_graph(make_sqlite_checkpointer(str(tmp_path / "cp.sqlite")))
    config = {
        "configurable": {"thread_id": "test:onboarding-year"},
        "recursion_limit": RECURSION_LIMIT,
    }
    messages = (
        [m for m, _ in ONBOARDING_TURNS[:3]]
        + ["in two years", "2027"]

...
| --- | --- |
| **One-word / no-answer input (≤ 2 words, or empty)** | Empty input gets the gentle hold ("Take your time — type as much or as little as you like."); 1–2-word answers get the specificity probes ("Tell me a bit more — what was YOUR role?"), up to 2 per question. Still no answer after the 2nd probe → score the composite as-is (0–3), move on. **3+ word answers are real answers — always judged, never probed.** All text from question to next question is the answer material the judge sees. |
